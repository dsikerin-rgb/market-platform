...
<head>
    ...
    @php($marketName = trim((string) ($invitation->market?->name ?? 'Эко Ярмарка')))
    ...
    <style>
        ...
        .hint{margin:6px 0 0;color:#6b7280;font-size:13px;line-height:1.4}
    </style>
</head>
<body>
<main class="page">
    <section class="card">
        <h1>Приглашение в команду «{{ $marketName }}»</h1>
        <p>Вы приглашены в команду «{{ $marketName }}».</p>
        ...
        <form method="post">
            ...
            @else
                ...
                <p class="hint">Этот пароль будет использоваться для входа в сервис.</p>
                @error('password')
                    <div class="error">{{ $message }}</div>
                @enderror
