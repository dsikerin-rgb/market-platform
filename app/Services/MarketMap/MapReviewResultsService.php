...
class MapReviewResultsService
{
    ...
    private function unconfirmedLinkClassification(
        ...
        if ($contractOverride !== null) {
            return [
                ...
                'tone' => 'success',
                'rank' => 5,
                'summary' => 'Текущее место уже подтверждается активным договором. Система всё ещё показывает проверку, потому что финансовый статус взят по арендатору, а не по точной цепочке договор → место.',
        ...
        if ($spaceGroupRole === MarketSpace::SPACE_GROUP_ROLE_CHILD) {
            return [
                ...
                'tone' => 'warning',
                'rank' => 8,
                'summary' => 'Это дочернее место группы. Для таких мест точная финансовая связь должна проверяться через основное место группы.',
                'recommended_action' => 'Откройте основное место/группу и проверьте, где фактически привязан договор 1С. Не разрешайте показывать долг на дочернем месте без этой проверки.',
                'evidence' => $evidence,
            ];
        }
        ...
        return [
            ...
            'recommended_action' => 'Сверьте договор 1С вручную. До подтверждения считайте это общим долгом арендатора, а не долгом конкретного места.',
            'evidence' => $evidence,
        ];
    }
    ...
    private function unconfirmedLinkEvidence(
        ...
        if ($spaceGroupRole === MarketSpace::SPACE_GROUP_ROLE_CHILD) {
            ...
            $evidence[] = $parentId > 0
                ? 'Дочернее место, основное #'.$parentId
                : 'Дочернее место без указанного основного';
        }
    ...
    private function settlementContractReviewForSpace(MarketSpace $space, int $marketId): array
    {
        ...
        $title = match ($state) {
            'current_space' => 'Договор ОСВ относится к этому месту',
            'other_space' => 'Договор ОСВ относится к другому месту',
            'multiple_spaces' => 'Договор ОСВ связан с несколькими местами',
            'unlinked_contract' => 'В договоре не указано место',
            'missing_local_contract' => 'Договор из ОСВ не найден в сервисе',
            'no_contract_in_osv' => 'В ОСВ нет договора для точной связи',
            default => 'Проверьте договор ОСВ',
        };
        $nextStep = match ($state) {
            'current_space' => 'Если договор действительно оформлен на это место, откройте договор и проверьте привязку.',
            'other_space' => 'Откройте место договора. Если договор относится туда, не связывайте его с текущей карточкой.',
            'multiple_spaces' => 'Откройте договор и проверьте, почему один договор связан сразу с несколькими местами.',
        ...
        return [
            ...
            'next_step' => $nextStep,
            'after_action' => 'Долг арендатора на карте не меняется. Здесь меняется только точная связь договора с местом.',
