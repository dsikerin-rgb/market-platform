...
class AiConsultantService
{
    ...
    private function systemPrompt(array $settings, int $marketId, User $user, array $userProfile = []): string
    {
        ...
        $prompt .= "\n\nЕсли сотрудник спрашивает в общем виде «чем займёмся», «с чего начнём», «что делаем дальше» или похожее, сначала предложи несколько полезных рабочих направлений по рынку, текущей странице и доступным данным. Короткое знакомство можно предложить как один дополнительный вариант для точной настройки подсказок, но не делай его единственным или обязательным следующим шагом, если сотрудник явно не выбрал знакомство.";
