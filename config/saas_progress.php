...
return [
    ...
    'current_focus' => 'Idempotent write adapter расширен до demo market, market_locations и tenants: demo:provision может создать или обновить только рынок, demo-локации и demo-арендаторов; остальные секции пока пропускаются.',
    'next_step' => 'Следующий малый пакет: расширить write adapter на spaces только для local/staging и только после отдельной проверки; prod data writes останутся под отдельным разрешением.',
    ...
    'stages' => [
        ...
        [
            ...
            'items' => [
                ...
                ['title' => 'demo:provision idempotent write adapter для local/staging: demo market, market_locations и tenants', 'status' => 'done'],
