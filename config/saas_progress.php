...
return [
    ...
    'current_focus' => 'Prod demo проверен super-admin: отдельный рынок id=2 работает без изменения боевого рынка id=1. Следующий пакет готовит простой вход с landing без публикации общего пароля, но оставляет public login за feature flag.',
    'next_step' => 'Задеплоить public demo entry с flags off, затем отдельно решить, где включать DEMO_PILOT_PUBLIC_LOGIN_ENABLED: сначала staging для проверки сценария, потом prod только после осознанного решения открыть demo-запись посетителям.',
    ...
    'stages' => [
        ...
        [
            ...
            'items' => [
                ...
                ['title' => 'Public demo entry подготовлен за feature flag: /demo может открыть synthetic demo-директора без публикации пароля, но flag по умолчанию выключен', 'status' => 'in_progress'],
        ...
        [
            ...
            'items' => [
                ...
                ['title' => 'Простой вход в demo с landing без общего пароля через feature flag', 'status' => 'in_progress'],
