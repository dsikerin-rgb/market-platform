...
if [ "$MISSING_COUNT" -gt 0 ]; then
    echo "=== INSERTING MISSING TENANTS (minimal columns) ==="
    MISSING_WHERE=$(cat /tmp/missing_tenants.txt | tr '\n' ',' | sed 's/,$//')
    
    # Insert with minimal common columns (avoiding JSON/text mismatches)
    ...
    " 2>&1 || {
        # If cross-database fails, use CSV export/import
        echo "Cross-database failed, using CSV export..."
...
CREATE TEMP TABLE missing_tenants_tmp (
    ...
    external_id varchar, one_c_uid varchar, slug varchar, created_at timestamp, updated_at timestamp
);
...
EOF
        rm -f /tmp/missing_tenants_export.csv
    }
