...
- `docs/ops.md` — окружения и пути (prod/staging), Redis-правила, staging deploy webhook, команды диагностики.
- `docs/ops-log.md` — журнал изменений (каждый сеанс: дата → что сделали → где/какие команды).

> Секреты/пароли не храним в репозитории и не публикуем в чате. Только в `.env` на сервере.
...
- **Staging**: автодеплой при изменениях в `main`, проверка “как на сервере”.
- **Production**: отдельный контур. Деплой сейчас выполняется вручную; при необходимости позже вводим политику “только по тегу релиза”.
...
- Prod: отдельный контур; политика деплоя уточняется (вручную / по тегу).
