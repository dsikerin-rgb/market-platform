...
class GenerateSanitaryDaysCommandSafetyTest extends TestCase
{
    public function test_command_source_uses_explicit_execute_guards_and_scheduled_execute(): void
    {
        $source = file_get_contents(app_path('Console/Commands/GenerateSanitaryDays.php'));
        ...
        self::assertStringContainsString('{--execute : Upsert sanitary days (default: dry-run)}', $source);
        ...
        self::assertStringContainsString("Schedule::command('market:calendar:generate-sanitary --execute --all-markets')", $schedule);
    }

    public function test_execute_and_dry_run_options_cannot_be_combined(): void
    {
        $this->artisan('market:calendar:generate-sanitary', [
            '--dry-run' => true,
            '--execute' => true,
        ])->assertFailed();
    }
