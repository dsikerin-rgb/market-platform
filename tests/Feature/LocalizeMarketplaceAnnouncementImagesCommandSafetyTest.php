...
class LocalizeMarketplaceAnnouncementImagesCommandSafetyTest extends TestCase
{
    public function test_command_source_uses_explicit_execute_and_market_context(): void
    {
        $source = file_get_contents(app_path('Console/Commands/LocalizeMarketplaceAnnouncementImagesCommand.php'));
        ...
        self::assertStringContainsString('{--execute : Apply image localization (default: dry-run)}', $source);
        ...
        self::assertStringContainsString('fn (): array => $this->localizeMarket($market, $overwrite, $dryRun),', $source);
        self::assertStringContainsString('if (! $dryRun) {', $source);
    }

    public function test_execute_and_dry_run_options_cannot_be_combined(): void
    {
        $this->artisan('marketplace:images:localize', [
            '--dry-run' => true,
            '--execute' => true,
        ])->assertFailed();
    }
