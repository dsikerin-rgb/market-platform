...
class SyncMarketHolidaysCommandSafetyTest extends TestCase
{
    public function test_command_source_uses_explicit_execute_guards_and_scheduled_execute(): void
    {
        $source = file_get_contents(app_path('Console/Commands/SyncMarketHolidays.php'));
        ...
        self::assertStringContainsString('{--execute : Upsert holidays from CSV (default: dry-run)}', $source);
        ...
        self::assertStringContainsString('MarketHoliday::query()->upsert(', $source);
        self::assertStringContainsString('DRY RUN: no holidays were upserted. Use --execute --market_id=... or --execute --all-markets to apply.', $source);
        self::assertStringContainsString("Schedule::command('market:holidays:sync --execute --all-markets')", $schedule);
    }

    public function test_execute_and_dry_run_options_cannot_be_combined(): void
    {
        $this->artisan('market:holidays:sync', [
            '--dry-run' => true,
            '--execute' => true,
        ])->assertFailed();
    }
