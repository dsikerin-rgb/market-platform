...
### 0.4) Группы мест: текущий статус

* MVP-блок групп мест закрыт на prod.
* Актуальный источник по группам мест хранится в active sources проекта как “Отчёт по группам мест — актуально”.
* В этом runbook больше не держим старый тезис про "ещё не внедряем" как текущую правду.
* Здесь остаются только правила safe auto-link и отображение статусов карты.
