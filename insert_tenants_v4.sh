...
STAGING_IDS=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME_STAGING" -Atc "SELECT string_agg(id::text, ',') FROM (SELECT id FROM tenants ORDER BY id) t;")
echo "Staging has $(echo "$STAGING_IDS" | tr ',' '\n' | wc -l) tenants"

# Get prod tenant IDs needed by financial tables
echo "=== GETTING PROD TENANT IDS NEEDED ==="
PROD_NEEDED=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME_PROD" -Atc "SELECT string_agg(id::text, ',') FROM (SELECT DISTINCT tc.tenant_id AS id FROM tenant_contracts tc WHERE tc.tenant_id IS NOT NULL UNION SELECT DISTINCT ta.tenant_id AS id FROM tenant_accruals ta WHERE ta.tenant_id IS NOT NULL ORDER BY id) t;")
